<script lang="ts">
  ...
  function draw() {
    ...
    const [baseW, baseH] = naturalAspect >= 1
  ...
  const toCropEvent = ([prev, curr]: [PointerMap, PointerMap]): CropEvent => {
    ...
    if (prevPtrs.length === 2 && currPtrs.length === 2) {
      const ratio = pinchDistance(currPtrs[0]!, currPtrs[1]!) /
        pinchDistance(prevPtrs[0]!, prevPtrs[1]!);
      return {
        kind: 'zoom',
        delta:
          crop.scale * (1 + (ratio - 1) * pinchZoomSensitivity) - crop.scale,
        transition: false,
      };
    }
  ...
  $effect(() => {
    ...
    const gesture$ = merge(
      ...
      merge(
        ...
        map((ev): PointerAction => ({ type: 'remove', id: ev.pointerId })),
      ),
    ).pipe(
    ...
    const wheel$ = fromEvent<WheelEvent>(canvas, 'wheel').pipe(
      ...
      map((ev): CropEvent => {
        if (ev.ctrlKey) {
          const delta = -ev.deltaY * trackpadPinchSensitivity *
            pinchZoomSensitivity;
          return { kind: 'zoom', delta, transition: false };
        }
        const rawPixels = ev.deltaMode === 1
          ? ev.deltaY * wheelZoomLineHeightPx
          : ev.deltaY;
        return {
          kind: 'zoom',
          delta: Math.max(
...
<style lang="scss">
  ...
  .zoom-fill {
    height: 100%;
    background-color: var(--trakttime-accent);
    border-radius: var(--border-radius-s);
